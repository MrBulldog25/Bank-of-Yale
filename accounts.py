...
class Account(Base):
    ...
    _account_number = Column(Integer, primary_key=True, autoincrement=False)
    _bank_id = Column(Integer, ForeignKey('banks._id'))
    _account_type = Column(String(20))
    ...
    def _assess_interest(self, latest_transaction, sesh):
...
class SavingsAccount(Account):
    ...
    _interest_rate = Column(DECIMAL(precision=5, scale=4), default=0.0041)
    _daily_limit = Column(Integer, default=2)
    _monthly_limit = Column(Integer, default=5)
...
class CheckingAccount(Account):
    ...
    """
    __tablename__ = 'checkingaccounts'
    _account_number = Column(Integer, ForeignKey('accounts._account_number'), primary_key=True)
    _interest_rate = Column(DECIMAL(precision=5, scale=4), default=0.0008)
    _balance_threshold = Column(Integer, default=100)
    _low_balance_fee = Column(DECIMAL(precision=5, scale=4), default=-5.44)

    __mapper_args__ = {
        'polymorphic_identity':'checkingaccounts'
    }
    ...
    def _assess_fees(self, latest_transaction, sesh):
